...
public class UserContactDataEndpoint extends UpdateableDomainDAO<UserContactData,Long> {
	...
	public String getCreatedAfterQuery() {
		return "FROM user_contact_data UCD WHERE UCD.user.id = :" +USER_ID_PARAMETER+ " AND UCD.createdAt > :" +CREATED_AT_PARAMETER;
	}

	@Override
	public String getUpdatedAfterQuery() {
		return "FROM user_contact_data UCD WHERE UCD.user.id = :" +USER_ID_PARAMETER+ " AND UCD.updatedAt > :" +UPDATED_AT_PARAMETER;
	}
