...
public class UserToProjectEndpoint extends UpdateableDomainDAO<UserToProject,Long> {
	...
	public String getCreatedAfterQuery() {
		return "SELECT DISTINCT UTP FROM user_to_project UTP WHERE UTP.user.id  = :" +USER_ID_PARAMETER  + " AND UTP.createdAt > :" +CREATED_AT_PARAMETER;
	}

	@Override
	public String getUpdatedAfterQuery() {
		return "SELECT DISTINCT UTP FROM user_to_project UTP WHERE UTP.user.id  = :" +USER_ID_PARAMETER  + " AND UTP.updatedAt > :" +UPDATED_AT_PARAMETER;
	}
