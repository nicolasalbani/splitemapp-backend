...
public class UserToGroupEndpoint extends UpdateableDomainDAO<UserToGroup,Long> {
	...
	public String getCreatedAfterQuery() {
		return "SELECT DISTINCT UTG FROM user_to_group UTG WHERE UTG.user.id  = :" +USER_ID_PARAMETER  + " AND UTG.createdAt > :" +CREATED_AT_PARAMETER;
	}

	@Override
	public String getUpdatedAfterQuery() {
		return "SELECT DISTINCT UTG FROM user_to_group UTG WHERE UTG.user.id  = :" +USER_ID_PARAMETER  + " AND UTG.updatedAt > :" +UPDATED_AT_PARAMETER;
	}
